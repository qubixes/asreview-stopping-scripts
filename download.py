...
from powertail import PowerTailNorm, ExpTailNorm
...
def main():
    ...
    result_fp = Path("output", "results.pkl")
    ...
    for file_name, part_url in files.items():
        ...
        compute_decision_function(file_name, url, model, feature_model,
                                  balance_model, all_results)
#         compute_distributions(file_name, all_results)

    optimize_power_tail(all_results, Path("output", "optimization.json"))
    return

    global_sorted_dist = get_ordered_distributions(all_results)
#     print(list(global_sorted_dist))
    with open(result_fp, "wb") as f:
        pickle.dump(all_results, f)


#     for dist_item in global_sorted_dist[:3]:

#     print(global_sorted_dist)
...
#         dist = dist_item["dist"]
#         one_dist = dist(loc=param[-2], scale=param[-1], *param[:-2])
#         plt.hist([df_one, df_zero], density=True)
#         plt.plot(x, one_dist.pdf(x), label=dist.name)
#     plt.legend()
#     plt.show()
# 
# #     data_fp = Path("data", file_name)
#     if not data_fp.is_file():
#         urlretrieve(base_url+part_url, data_fp)
# 
...
def compute_decision_function(file_name, url, model, feature_model,
...
#         all_df_one.extend(df_one.tolist())
#         bounds.append(bounds[-1]+len(df_one))

#     all_df_one = np.array(all_df_one)

#     print(bounds, all_df_one)
...
#     dist = PowerTailNorm(*param)
    dist_exp = ExpTailNorm(*param)
#     print(self_check(dist_exp))
#     print(self_check(dist))
    x_range = np.linspace(-6, 6)
    plt.hist(df_one, 40, density=True)
#     plt.plot(x_range, dist.pdf(x_range), label="Power tail")
...
def get_ordered_distributions(result):
    ...
    global_sorted_dist = {k: global_dist[k] for k in sorted(global_dist, key=get_total_rank)}
...
#         print()
#     print(all_dist[:10])
#             print(dist.name, len(param), ks_p)

            
#     a, loc, scale = skewnorm.fit(df_one)

#     ks_stat, ks_p = kstest(df_one, skewnorm.cdf, (a, loc, scale))



#     print(ks_p)
#     one_distrib = skewnorm(a, loc=loc, scale=scale)
#     print()


#     plt.hist([df_one, df_zero], density=True)
#     plt.plot(x, one_distrib.pdf(x))
#     plt.show()
#     print(one_idx)
